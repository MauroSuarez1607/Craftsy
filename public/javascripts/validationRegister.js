  ...
  $("surname").addEventListener("blur", function (e) {
    switch (true) {
      case !this.value.trim():
        ...
        break;
      case this.value.trim().length < 2:
        $("msgError-surname").innerHTML = "Minimo dos letras";
        this.classList.add("is-invalid");
        break;
      case !/^[ a-zA-ZñÑáéíóúÁÉÍÓÚ]+$/.test(this.value.trim()):
  ...
  $("email").addEventListener("blur", function (e) {
    switch (true) {
      ...
      case !/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(this.value.trim()):
        $("msgError-email").innerHTML = "El formato es invalido";
        this.classList.add("is-invalid");
        break;
      default:
        $("msgError-email").innerHTML = null;
  ...
  $("password").addEventListener("blur", function (e) {
    switch (true) {
      ...
      case !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{6,12}$/.test(this.value.trim()):
        $("msgError-password").innerHTML = "La contraseña debe tener entre 6 y 12 caracteres, minuscula, mayuscula, numero y caracter especial";
        this.classList.add("is-invalid");
        break;
      default:
        $("msgError-password").innerHTML = null;
