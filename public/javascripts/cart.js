...
const showProductInCart = (products, total) => {
  if ($("cart-table")) {
    ...
    products.forEach(({ id, image, name, price, discount, quantity }) => {
      ...
      <tr>
      <th scope="row"><img src="/images/${image}" width=100 alt="hola">
      </th>
      <td>${name}</td>
      <td>
        <div class="d-flex gap-2">
          <button onclick="removeItemToCart(${id})" class="btn btn-sm btn-danger"><i class="fas fa-minus"></i></button>
          <input type="text" value="${quantity}" style="width:30px;text-align:center">
          <button onclick="addItemToCart(1, ${id})" class="btn btn-sm btn-success"><i class="fas fa-plus"></i></button>
        </div>
      </td>
